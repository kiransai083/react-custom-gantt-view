...
const AiDEChat = ({ type, botData, promptOrder, defaultConv }) => {
    console.log("🚀 ~ file: AiDEChat.jsx:32 ~ AiDEChat ~ type:", type, botData, defaultConv);
    ...
    const getSequence = () => {
        defaultConv[0].prompt
    };

    const prepareConversation = () => {
        const setNextConverstion = (defaultConv || []).find((conv) => conv.order == promptOrder);
        
        const isDuplicate = conversation[conversation?.length - 1]?.sequenceId == promptOrder && promptOrder !=0;

        switch (type) {
            case 'brief':
                setConversation([...conversation, ...(botData?.prompt && !isDuplicate ? [{
                    "from": "BOT",
                    "text": setNextConverstion?.prompt || '',
                    "list": setNextConverstion?.list || [],
                    "sequenceId" :promptOrder
            ...
            case 'about':
            case 'approvals':
                setConversation([...conversation, ...( !isDuplicate ? [{
                    "from": "BOT",
                    "text": setNextConverstion?.prompt || '',
                    "list": setNextConverstion?.list || [],
                    "sequenceId" :promptOrder
    ...
    const publishConv = (conv) => {

        const message = {
            ...botData,
            path: botData?.path,
            type: botData?.type,
            prompt: conv[conv.length - 1],
            sequence: promptOrder
        };
            ...
            <Flex flex="0.9" direction={"column"} alignItems="flex-end">
                ...
                {conversation.length == 0 && type === 'brief' && <Flex mt="10px" direction={"column"} width="100%" alignItems={"center"} justifyContent={"center"} >
                    ...
                    <Flex p="1rem" boxSizing="border-box" overflowY={"scroll"} direction={"column"} width="100%">
                        {[{
                            "from": "BOT",
                            "text": "Hello! \n Thank you for choosing Brief",
                            "attachment": ''
                        }, {
                            "from": "BOT",
                            "text": "Let me know how can I assit you, \n Currently, I can help you through the following processes",
                            "attachment": ''
                ...
                {conversation.length == 0 && (!type || type === 'default') && <Flex mt="10px" direction={"column"} width="100%" alignItems={"center"} justifyContent={"center"} >
                                        ...
                                        <Flex direction={"column"} alignItems="flex-start" width={"100%"}>
                                            ...
                                            {item?.list?.map((label) => {
                                                ...
                                                return (
                                                    ...
                                                    <Text key={label} mt="0"  onClick={() => onOptionClick(label)} backgroundColor={"F4F4ED"} cursor="pointer" borderRadius="5px" color="#29292A" p="0.2rem 0.5rem" border="1px solid #29292A" ml="1rem">
                                                        {label}
                                                    </Text>
